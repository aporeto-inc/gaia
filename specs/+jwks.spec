# Model
model:
  rest_name: jwks
  resource_name: jwks
  entity_name: JWKS
  package: squall
  group: internal/oauth
  description: |-
    The JWKS object contains the set of parameters that are read by an OIDC
    compliant server to validate a JWT issued by an OIDC provider.
  private: true
  detached: true

# Attributes
attributes:
  v1:
  - name: alg
    description: Alg defines the algorithm used for signing as per the JWKS specification.
    type: enum
    exposed: true
    read_only: true
    allowed_choices:
    - RS256
    - ES256
    autogenerated: true

  - name: curve
    description: Curve is the curve type used for signing. Valid only when signing
      method is EC.
    type: enum
    exposed: true
    read_only: true
    allowed_choices:
    - P256
    autogenerated: true

  - name: kid
    description: kid is the key ID as per the JWKS specification.
    type: string
    exposed: true
    read_only: true
    autogenerated: true

  - name: kty
    description: kty defines the key type as per the JWKS specification.
    type: enum
    exposed: true
    read_only: true
    allowed_choices:
    - RSA
    - EC
    autogenerated: true

  - name: modulo
    description: |-
      Modulo is the modulo value of an RSA public key. Valid only when the signing
      method is RSA.
    type: string
    exposed: true
    read_only: true
    autogenerated: true

  - name: use
    description: Use defines the use of the signing key as per the JWKS specification.
    type: string
    exposed: true
    read_only: true
    autogenerated: true

  - name: x
    description: X is the X value of the public key. Valid only when signing method
      is EC.
    type: string
    exposed: true
    read_only: true
    autogenerated: true

  - name: "y"
    description: Y is the Y value of the public key. Valid only when signing method
      is EC.
    type: string
    exposed: true
    read_only: true
    autogenerated: true
