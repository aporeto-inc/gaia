...
##### `affectedEndpoints` `[]string` [`autogenerated`,`read_only`]
...
##### `status` `enum(Degraged | Fail | Pass)` [`autogenerated`,`read_only`]

The current health of the observed sub system.

##### `type` `enum(Cache | Database | General | MessageBus | Service | TSDB)` [`autogenerated`,`read_only`]
