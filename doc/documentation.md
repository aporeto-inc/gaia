...
##### `claimsType` `enum(Transmitted | Received)` [`required`]
...
##### `payloadSizeType` `enum(Transmitted | Received)` [`required`]
...
##### `remoteEndpointType` `enum(ProcessingUnit | External)` [`required`]
...
##### `remoteNamespaceType` `enum(Plain | Hash)` [`required`]
...
##### `type` `enum(Request | Response)` [`required`]
