...
func NewRenderedPolicy() *RenderedPolicy {
	...
	return &RenderedPolicy{
		ModelVersion:            1,
		DefaultIncomingPUAction: RenderedPolicyDefaultIncomingPUActionDeny,
		DefaultOutgoingPUAction: RenderedPolicyDefaultOutgoingPUActionDeny,
...
func (o *RenderedPolicy) Validate() error {
	...
	if err := elemental.ValidateStringInList("defaultIncomingPUAction", string(o.DefaultIncomingPUAction), []string{"Deny", "Allow"}, false); err != nil {
		errors = errors.Append(err)
	}

	if err := elemental.ValidateStringInList("defaultOutgoingPUAction", string(o.DefaultOutgoingPUAction), []string{"Deny", "Allow"}, false); err != nil {
		errors = errors.Append(err)
	}
...
owning the datapath in this case. It is merely providing an authorizer API.`,
	...
	"DefaultIncomingPUAction": {
		...
		DefaultValue:   RenderedPolicyDefaultIncomingPUActionDeny,
	...
	"DefaultOutgoingPUAction": {
		...
		DefaultValue:   RenderedPolicyDefaultOutgoingPUActionDeny,
